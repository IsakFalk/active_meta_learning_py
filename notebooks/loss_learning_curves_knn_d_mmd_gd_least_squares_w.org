...
* KNN active meta learning
Active meta learning using KNN with no cheating. The distance measure for KNN is
MMD and the prototype for \(w_{0}\) is the ridge estimate from the dataset that
the task generated in the order: generate \(w \sim \mu\) which generates a
dataset \(D\). This dataset gives rise to an estimate \(\hat{w}\) which is the
ridge regression solution given some regularisation parameters \(\lambda\).

** Setting up environment
Stdlib, installed and local
...
Below we define the actual workhorse, the ~MetaKNNGD~ class which performs
everything at once, calculating the ridge regression estimates, fits and the
loss matrix \(\matr{M}_{tj} = L(D^{val}_{j}, \theta_{t})\) where \(\theta_{t}\)
is the (meta)-algorithm after running \(t\) steps of the active learning meta
algorithm.
#+begin_src jupyter-python
class MetaKNNGDExperiment:
    ...
    def _reorder(self):
       ...
       self.M_tr_te = self.M_tr_te_original[np.ix_(self.train_task_reordering, np.arange(self.T_te))]
...
** Experiment
*** Setting
We define the setting, nfeel free to tweak if you want to try another
...
meta_val_batches = 200
meta_val_batch_size = 1  # Hardcoded
meta_test_batches = 300
meta_test_batch_size = 1  # Hardcoded
env = HypercubeWithKVertexGaussian(d, k=k_mixtures, s2=0.0)
mus, kappas, ps = create_vmf_params(d, k_mixtures, kappa)
env = VonMisesFisherMixture(mus, kappas, ps)
#+end_src

*** Create batches
Load train, val and test batches (we won't run cross-validation here)
#+begin_src jupyter-python
logging.info("Generating meta-train batches")
# Sample meta-train
train_dataset = EnvironmentDataSet(k_shot, k_query, env, noise_w=noise_w, noise_y=noise_y)
...
Calculate the median base and meta lengthscale for double mmd kernel
#+begin_src jupyter-python
...
and then the distance between all of the datasets in the MMD sense
#+begin_src jupyter-python
M_tr_val = np.sqrt(_mmd2_matrix(
    train_datasets, val_datasets, base_s2_D
))
M_tr_te = np.sqrt(_mmd2_matrix(
    train_datasets, test_datasets, base_s2_D
))
#+end_src

Quickly check the histogram to see that everything is reasonable
#+begin_src jupyter-python :tangle no
plt.hist(M_tr_te.ravel())
#+end_src

*** Generate ordering
Run KH for weights and data together with baseline (uniform sampling). This
gives us the ordering which will be used for getting the active meta
learning prototype set
#+begin_src jupyter-python
...
K_D = _gaussian_kernel_mmd2_matrix(train_datasets, train_datasets, base_s2_D, meta_s2_D)
...
*** Defining models and getting losses
#+begin_src jupyter-python
...
Cross validating to get optimal \(\alpha, \lambda\)
#+begin_src jupyter-python
...
#+begin_src jupyter-python
plt.imshow(experiment_data["uniform"]["test_error"])
plt.colorbar()
#+end_src

#+begin_src jupyter-python
plt.imshow(experiment_data["kh_weights"]["test_error"])
plt.colorbar()
#+end_src

#+begin_src jupyter-python
plt.imshow(experiment_data["kh_data"]["test_error"])
plt.colorbar()
#+end_src
...
plot_sns_tsplot(meta_test_error, ax, until_t=None)
#+end_src
