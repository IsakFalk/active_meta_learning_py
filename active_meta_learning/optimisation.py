...
class KernelHerding:
    ...
    def _objective_func(self, t):
        ...
        assert len(self._sampled_indices) == t
        K_nn = self.K
        K_Qn = self.K[self._sampled_indices]
        tp1 = len(self._sampled_indices) + 1
        ...
        J = (1.0 / self.n) * K_nn.sum(axis=0) - (1.0 / tp1) * K_Qn.sum(axis=0)
        assert J.shape == (
            self.n,
    ...
    def run_kernel_herding(self, start_idx=0):
        ...
        self._sampled_indices.append(start_idx)
        ...
        for t in range(1, self.stop_t):
            ...
            J_argmax = J.argmax()
            self._sampled_indices.append(J_argmax)
            self._objective_curve[t] = J[J_argmax]

        self.sampled_order = np.array(self._sampled_indices)
        return self.sampled_order
