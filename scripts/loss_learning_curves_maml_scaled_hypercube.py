...
from active_meta_learning.data import (
    ...
    HypercubeScaledAxesWithKVertexGaussian
)
...
if __name__ == "__main__":
    ...
    p_hi = 0.2
    axes_hi_range = (10, 20)
    axes_low_range = (0.5, 3.0)
    axes_scales = np.ones(d)
    for i in range(d):
        if random.uniform(0.0, 1.0) < p_hi:
            axes_scales[i] = random.uniform(*axes_hi_range)
        else:
            axes_scales[i] = random.uniform(*axes_low_range)
    env = HypercubeScaledAxesWithKVertexGaussian(d, k, noise_w, mixture_vertices=None, axes_scales=axes_scales)
