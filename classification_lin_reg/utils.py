...
import kernels
from data import LeoEmbeddingDataset
...
def convert_batches_to_fw_form(sampled_batches):
    with torch.no_grad():
        ...
        sampled_batches_x = make_X_to_D(sampled_batches)
        sampled_batches_y = make_Y_to_D(sampled_batches)
        sampled_batches_x = remove_batched_dimension_in_D(sampled_batches_x)
        sampled_batches_y = remove_batched_dimension_in_D(sampled_batches_y)
        sampled_batches_x = convert_batches_to_np(sampled_batches_x)
        sampled_batches_y = convert_batches_to_np(sampled_batches_y)
        return np.stack(sampled_batches)


def generate_lin_reg_kernel_form_from_Xy_dict_form(sampled_batches):
    M_x = (
        torch.stack([d["X"] for d in sampled_batches])
        .flatten(start_dim=1, end_dim=-2)
        .numpy()
    )
    M_y = (
        torch.stack([d["y"] for d in sampled_batches])
        .flatten(start_dim=1, end_dim=-2)
        .to(torch.int64)
    )
    M_y = F.one_hot(M_y).numpy().squeeze()
    return M_x, M_y
...
def get_active_learning_batches_lin_reg(sampled_batches_train, fw_class):
    _sampled_batches = coalesce_train_and_test_in_dicts(sampled_batches_train)
    M_x, M_y = generate_lin_reg_kernel_form_from_Xy_dict_form(_sampled_batches)
    K = kernels.lin_reg_gaussian_kernel_matrix(M_x, M_y)
    fw = fw_class(K)
    fw.run()
    sampled_batches_train_fw = [sampled_batches_train[i] for i in fw.sampled_order]
    return sampled_batches_train_fw
...
def generate_dataloaders_leo(args):
    dataset_train = LeoEmbeddingDataset(
        args.data_path,
        dataset_name="miniImageNet",
        dataset_split=args.base_dataset_train,
        embedding_crop="center",
        train_on_val=False,  # use train+val into bigger train set
        n_way=args.n_way,
        k_shot=args.k_shot,
        k_query=args.k_query,
        total_examples_per_class=600,  # this to reproduce miniimagenet composition
    )
    dataset_val = LeoEmbeddingDataset(
        args.data_path,
        dataset_name="miniImageNet",
        dataset_split=args.base_dataset_val,
        embedding_crop="center",
        train_on_val=False,  # use train+val into bigger train set
        n_way=args.n_way,
        k_shot=args.k_shot,
    ...
    dataset_test = LeoEmbeddingDataset(
        args.data_path,
        dataset_name="miniImageNet",
        dataset_split=args.base_dataset_test,
        embedding_crop="center",
        train_on_val=False,  # use train+val into bigger train set
        n_way=args.n_way,
        k_shot=args.k_shot,
    ...
    dataloader_train = DataLoader(
        ...
        batch_size=args.tasks_per_metaupdate,
        num_workers=args.n_workers,
        collate_fn=dataset_train._embedding_collate_fn,
    )
    dataloader_val = DataLoader(
        dataset_val,
        batch_size=args.tasks_per_metaupdate,
        num_workers=args.n_workers,
        collate_fn=dataset_val._embedding_collate_fn,
    )
    dataloader_test = DataLoader(
        dataset_test,
        batch_size=args.tasks_per_metaupdate,
        num_workers=args.n_workers,
        collate_fn=dataset_test._embedding_collate_fn,
    )
    return dataloader_train, dataloader_val, dataloader_test
